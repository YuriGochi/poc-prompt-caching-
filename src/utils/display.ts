...
// gpt-4o pricing (2025): $2.50/1M input, $1.25/1M cached input, $10.00/1M output
const PRICE_INPUT_PER_TOKEN = 2.50 / 1_000_000;
const PRICE_CACHED_PER_TOKEN = 1.25 / 1_000_000;
...
export function printSummary(results: TurnResult[], label: string) {
  ...
  const avgTtf = results.reduce((s, r) => s + r.ttfTokenMs, 0) / results.length;
  const avgHitRate = results.reduce((s, r) => s + r.cacheHitRate, 0) / results.length;
  console.log('─'.repeat(96));
  console.log(`  ${label}`);
  console.log(`  Avg TTFT    : ${avgTtf.toFixed(0)}ms`);
